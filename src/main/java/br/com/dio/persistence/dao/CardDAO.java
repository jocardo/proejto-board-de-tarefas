    ...
    public Optional<CardDetailsDTO> findById(final Long id) throws SQLException {
        var sql = """
                      SELECT c.id, c.title, c.description,
                             ...
                             c.board_column_id,
                             bc.name, 
                             (SELECT COUNT(sub_b.id) FROM BLOCKS AS sub_b WHERE sub_b.card_id = c.id) AS blocks_amount
                      FROM CARDS AS c
                      LEFT JOIN BLOCKS AS b ON c.id = b.card_id AND b.unblocked_at IS NULL
                      INNER JOIN BOARD_COLUMNS AS bc ON c.board_column_id = bc.id
                        WHERE c.id = ?
                   """;
